...
//이름, 판매가, 매입가, 재고, 안전재고, 판매량, Account 클래스  
class Coffee {
	...
	public int registerSales(int salesCnt) {
		...
		//0개를 팔아서 고객에게 받을 돈이 0일 경우 
		int payPrice = salesPrice * deductStock(salesCnt);
		
		//결제금액이 0이 아니라면 판매가 정상적으로 이루어진 상황일 경우 
		if(payPrice > 0) {
			//누적판매량 추가 
			salesCount += salesCnt;
		}
	...
	//반품등록 메서드 
	public int qksvna(int qksvnaCnt) {
		int qksvnaPrice = salesPrice * addStock(qksvnaCnt, salesPrice);
		if(qksvnaPrice > 0) {
			salesCount -= qksvnaCnt; 
		}
		return qksvnaPrice;
	...
	private int deductStock(int coffeeCnt) {
		...
		//재고가 충분하지 않아서 재고를 추가하고 차감해야 하는 경우
			...
			stock -= coffeeCnt;
			return coffeeCnt;
		//재고가 충분하지 않은데 잔고가 부족해서 재고추가가 불가능한 경우 	
		}else {
			return 0;
		}		
	}
	...
	private void secoureSfetyStock() {
	...
	private int addStock(int coffeeCnt, int price) {
		//비용산정 
		int expencesPrice = coffeeCnt * price; // <-매입가 
